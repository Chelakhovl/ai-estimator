...
def _allows_labour_markup_override(prompt: str) -> bool:
    return bool(
        re.search(
            r"\b(labou?r markup|labou?r margin|worker markup)\b", prompt, re.IGNORECASE
        )
    )
...
def _score_segment_against_row(
    segment: str, row: CandidateRow, requested_unit: str | None
) -> float:
    segment_tokens = expand_tokens(tokenize(segment))
    work_tokens = expand_tokens(tokenize(row.WorkName))
    context_tokens = expand_tokens(
        tokenize(
            " ".join(
                filter(
                    None,
    ...
    overlap = len(segment_tokens & work_tokens) * 2 + len(
    ...
    if (
        overlap > 0
        and requested_unit
        and normalized_row_unit
        and normalized_row_unit != requested_unit
    ):
        score -= 0.75
    return score
...
def _resolve_shortlist_limit(
    raw_prompt: str, extracted_scope, *, candidate_count: int = 0
) -> int:
    ...
    if candidate_count > _DYNAMIC_SHORTLIST_BASE_LIMIT:
        candidate_scale_bonus = min(
            (candidate_count - _DYNAMIC_SHORTLIST_BASE_LIMIT)
            // _DYNAMIC_SHORTLIST_CANDIDATE_BONUS_DIVISOR,
...
def _build_examples_budget_assumption(
    *,
    ...
    action = (
        "omitted"
        if final_count == 0
        else f"reduced from {original_count} to {final_count}"
    )
    return PreviewAssumption(
        text=(
...
def _fit_accepted_examples_to_budget(
    *,
    ...
    if baseline_chars <= _PREVIEW_ROUGH_CHAR_BUDGET and not (
        ...
        and baseline_chars
        > int(_PREVIEW_ROUGH_CHAR_BUDGET * _PREVIEW_ACCEPTED_EXAMPLE_PRESSURE_RATIO)
    ):
...
def _fit_shortlist_to_budget(
    *,
    ...
    while (
        len(current) > _PREVIEW_HARD_MIN_SHORTLIST_SIZE
        and message_chars > _PREVIEW_ROUGH_CHAR_BUDGET
    ):
        next_size = max(
            _PREVIEW_HARD_MIN_SHORTLIST_SIZE,
            len(current) - _PREVIEW_SHORTLIST_TRIM_STEP,
        )
...
def _retry_trimmed_shortlist(
    *,
    ...
    next_size = max(
        _PREVIEW_HARD_MIN_SHORTLIST_SIZE, len(shortlist) - _PREVIEW_SHORTLIST_TRIM_STEP
    )
    ...
    if code == "do_not_include":
        ...
        return (
            ...
            PreviewUnmatchedItem(
                ...
                reason=reason
                or "This item is marked do_not_include and should not be auto-added to the quote.",
            ),
        )
...
def _apply_specialist_trade_review(
    ...
    *,
    ...
    if "ufh" in tokens or ("underfloor" in tokens and "heating" in tokens):
        ...
        specialist_trade_tags.append("plumbing-heating")
    if "gas" in tokens and any(
        token in tokens for token in {"line", "pipe", "pipework"}
    ):
        specialist_reasons.append(
    ...
    if specialist_reasons:
        ...
        for reason in specialist_reasons:
            matched_row.ReviewReason = _append_review_reason(
                matched_row.ReviewReason, reason
            )
        scope_label = "specialist trade"
        if (
            "electrical" in specialist_trade_tags
            and "plumbing-heating" in specialist_trade_tags
        ):
...
def _apply_hourly_unit_review(
    ...
    *,
    ...
    if any(hint in lowered_name for hint in _GENERIC_HOURLY_NAME_HINTS):
        review_reason = "This match uses a generic hourly row. Prefer a more specific counted or measured row if one exists in the catalog."
    elif section_key == "electrical" and any(
        keyword in lowered_scope
        for keyword in (
            "switch",
            "socket",
            "spotlight",
        ...
        review_reason = "Electrical point scope matched to an hourly row. Confirm whether a point-based electrical row should be used instead."
        ...
        review_reason = "Plumbing / heating scope matched to an hourly row. Confirm whether a point-based or measured row should be used instead."
    elif (
        section_key in {"flooring", "tiling", "plastering", "ceilings_and_insulation"}
        and matched_row.QUANTITY >= 8
    ):
        review_reason = "Area-driven finish scope matched to an hourly row. Confirm whether an m2-based row should be used instead."
    elif section_key in {"joinery", "carpentry", "woodwork_painting"} and any(
        keyword in lowered_scope
        for keyword in ("skirting", "architrave", "window board")
    ):
        review_reason = "Joinery finish scope matched to an hourly row. Confirm whether a length- or count-based row should be used instead."
    elif section_key == "ceilings_and_insulation" and any(
        keyword in lowered_scope
        for keyword in ("insulation", "rockwool", "pir", "acoustic")
    ):
        review_reason = "Insulation scope matched to an hourly row. Confirm whether an m2-based insulation row should be used instead."
    ...
    matched_row.ReviewReason = _append_review_reason(
        matched_row.ReviewReason, review_reason
    )
    assumptions.append(
        PreviewAssumption(
...
def _build_preview_telemetry(
    ...
    *,
    ...
    return PreviewTelemetry(
        ...
        if accepted_example_count is None
        else accepted_example_count,
        ...
        structured_output_mode=str(
            llm_metadata.get("structured_output_mode", "") or ""
        ),
        fallback_reason=_truncate_fallback_reason(fallback_reason),
        takeoff_heuristics_version=TAKEOFF_HEURISTICS_VERSION
        if extracted_scope is not None
        else "",
        takeoff_heuristics_source=TAKEOFF_HEURISTICS_SOURCE
        if extracted_scope is not None
        else "",
        rule_version=rules_context.rule_version if rules_context is not None else "",
        canonical_source=rules_context.canonical_source
        if rules_context is not None
        else "",
        runtime_source=rules_context.runtime_source
        if rules_context is not None
        else "",
        published_version=rules_context.published_version
        if rules_context is not None
        else "",
        published_at=rules_context.published_at or ""
        if rules_context is not None
        else "",
    )
...
def _build_extracted_scope(prompt: str):
    try:
        ...
        logger.exception(
            "Structured scope extraction failed during preview generation."
        )
        return None
...
def _resolve_preview_input(request: PreviewRequest) -> tuple[str, object | None, str]:
    if _has_normalized_input(request):
        normalized_prompt = (
            request.normalized_prompt_markdown or request.prompt
        ).strip()
        extracted_scope = request.normalized_scope or _build_extracted_scope(
            normalized_prompt
        )
        ...
        if extracted_scope is not None and not extracted_scope.sections:
            ...
            if prompt_scope and prompt_scope.sections:
                extracted_scope = extracted_scope.model_copy(
                    update={"sections": prompt_scope.sections}
                )
...
def _build_coverage_summary(
    ...
    *,
    ...
    matched_section_count = len(
        {row.MatchedSectionKey for row in matched_rows if row.MatchedSectionKey}
    )
    section_count = 0
    room_count = 0
    if extracted_scope is not None:
...
def _build_review_queue(
    ...
    for row in matched_rows:
        ...
        reason = (
            (row.ReviewReason or "").strip()
            or "This matched row still needs estimator review before it is safe to apply."
        )
        ...
        register(
            PreviewReviewQueueItem(
                code=_normalize_queue_code(
                    f"row-{row.WORKGUID or row.WorkName}", fallback="row-review"
                ),
                title=title,
                message=reason,
                severity="warning",
    ...
    for item in unmatched_items:
        ...
        register(
            PreviewReviewQueueItem(
                code=_normalize_queue_code(
                    f"unmatched-{source_text}", fallback="unmatched-scope"
                ),
                title=source_text,
                message=item.reason
                or "This scope line was not matched into a priced row.",
                severity="warning",
                kind="missing_scope",
    ...
    for assumption in assumptions:
        ...
        register(
            PreviewReviewQueueItem(
                code=_normalize_queue_code(
                    f"{kind}-{text[:48]}", fallback=kind or "review"
                ),
...
def _build_coverage_prompts(
    ...
    for item in unmatched_items:
        ...
        register(
            PreviewCoveragePrompt(
                code=_normalize_queue_code(
                    f"coverage-{source_text}", fallback="coverage-item"
                ),
                title=source_text,
    ...
    for assumption in assumptions:
        ...
        register(
            PreviewCoveragePrompt(
                code=_normalize_queue_code(
                    f"{kind}-{text[:48]}", fallback=kind or "coverage"
                ),
...
def _generate_mock_preview(request: PreviewRequest) -> PreviewResponse:
    ...
    shortlist_prompt = (
        f"{prompt}, {build_scope_matching_context(extracted_scope)}"
        if extracted_scope
        else prompt
    )
    ...
    shortlist = shortlist_candidates(
        shortlist_prompt, request.candidate_rows, limit=shortlist_limit
    )
    ...
    for segment in segments:
        ...
        resolved_quantity = quantity or (
            row.QUANTITY if row.QUANTITY and row.QUANTITY > 0 else 1
        )
        ...
        takeoff_suggestion = (
            suggest_quantity_from_takeoff(row, extracted_scope, segment)
            if extracted_scope
            else None
        )
        ...
        if quantity is None:
            if (
                takeoff_suggestion
                and takeoff_suggestion.quantity
                and takeoff_suggestion.quantity > 0
            ):
                ...
                if takeoff_suggestion.needs_review:
                    needs_review = True
                confidence = max(
                    confidence, 0.78 if not takeoff_suggestion.needs_review else 0.7
                )
                if takeoff_suggestion.review_reason:
                    review_reasons.append(takeoff_suggestion.review_reason)
            else:
                if normalized_row_unit in _MEASURED_QUANTITY_REQUIRED_UNITS and not (
                    row.QUANTITY and row.QUANTITY > 0
                ):
                    assumptions.append(
                        PreviewAssumption(
                ...
                review_reasons.append(
                    "Quantity was not found in the prompt, so a default value was used."
                )

        if (
            requested_unit
            and normalized_row_unit
            and requested_unit != normalized_row_unit
        ):
            assumptions.append(
                PreviewAssumption(
                    text=(
            ...
            needs_review = True
            review_reasons.append(
                f"Prompt unit {requested_unit} does not match the row unit {normalized_row_unit}."
            )

        if needs_review and not review_reasons:
            review_reasons.append(
                "Low-confidence match. Please review before applying."
            )

        section_match = (
            match_row_to_extracted_section(row, extracted_scope, segment)
            if extracted_scope
            else None
        )
        ...
        should_skip, guardrail_assumptions, guardrail_unmatched = (
            _apply_guardrail_policy(
                matched_row,
                row=row,
            )
        )
    ...
    return PreviewResponse(
        ...
        review_summary=_build_review_summary(
            matched_rows, unmatched_items, assumptions
        ),
...
def _generate_llm_preview(
    request: PreviewRequest, client: LLMClient
) -> PreviewResponse:
    source_prompt, extracted_scope, input_mode = _resolve_preview_input(request)
    prompt = normalize_prompt(source_prompt)
    shortlist_prompt = (
    ...
    while True:
        try:
            ...
            if "extracted_scope" in error_text or "accepted_examples" in error_text:
                llm_output = client.preview_match(
                    prompt=source_prompt, candidate_rows=shortlist
                )
                break
            raise
    ...
    for llm_row in llm_output.matched_rows:
        ...
        if (
            (llm_row.QUANTITY is None or llm_row.QUANTITY <= 0)
            and extracted_scope
            and candidate
        ):
            ...
            takeoff_suggestion = suggest_quantity_from_takeoff(
                candidate, extracted_scope, segment_hint
            )
        ...
        if (
            ...
            matched_row.Confidence = max(
                matched_row.Confidence,
                0.78 if not takeoff_suggestion.needs_review else 0.7,
            )
            if takeoff_suggestion.needs_review:
                matched_row.NeedsReview = True
        ...
        if extracted_scope:
            if candidate:
                ...
                segment_hint = " ".join(
                    filter(None, [llm_row.AREA, candidate.WorkName])
                )
...
def generate_preview(request: PreviewRequest) -> PreviewResponse:
    ...
    if client.is_enabled():
        try:
            ...
            if not settings.allow_mock_fallback:
                raise RuntimeError(
                    "LLM preview failed and mock fallback is disabled in this environment."
                ) from exc
            fallback = _generate_mock_preview(request)
            fallback_kind = (
                "normalized_service_fallback"
                if _has_normalized_input(request)
            ...
            fallback.telemetry.parse_retry_used = bool(
                llm_metadata.get(
                    "parse_retry_used", fallback.telemetry.parse_retry_used
                )
            )
            fallback.telemetry.structured_output_mode = str(
                llm_metadata.get(
                    "structured_output_mode", fallback.telemetry.structured_output_mode
                )
                or fallback.telemetry.structured_output_mode
            )
            fallback.telemetry.fallback_reason = _truncate_fallback_reason(
                f"{type(exc).__name__}: {exc}"
            )
            if fallback.telemetry.room_count > 0:
                fallback.telemetry.takeoff_heuristics_version = (
                    TAKEOFF_HEURISTICS_VERSION
                )
    ...
    if not settings.allow_mock_fallback:
        raise RuntimeError(
            "LLM preview is not configured and mock mode is disabled in this environment."
        )
